...
//	}
    ...
    /**
     * Set resources info,symbolgroup can be null
     */
    public void setResourcesInfo(Resources resources, SymbolGroup symbolGroup) {
        fireResourcesChanged(new ResourcesChangedEvent(resources,symbolGroup));
    }

    private void fireResourcesChanged(ResourcesChangedEvent e) {
        Vector<ResourcesChangedListener> listeners = resourcesChangedListeners;
        for (ResourcesChangedListener listener:listeners) {
            listener.resourcesChanged(e);
        }
    }

    public void addResourcesChangedListener(ResourcesChangedListener listener) {
        if (null != listener && !resourcesChangedListeners.contains(listener))
            resourcesChangedListeners.add(listener);
    }

    public void removeResourcesChangedListener(ResourcesChangedListener listener) {
        if (null != listener && resourcesChangedListeners.contains(listener))
            resourcesChangedListeners.remove(listener);
    }

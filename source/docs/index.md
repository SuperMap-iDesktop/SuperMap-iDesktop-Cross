...
-   **地统计分析**：分析和预测与空间或时空现象相关的统计值，包括度量地理分析、模型分析、聚类分布、空间关系建模等，可用于分析数据的要素间的聚合、离散关系、结构和趋势等。

### 工作流

　　工作流支持对数据导入、处理、分析等功能进行建模，用户可根据需要设计可连续执行数据处理的工作流，实现了一键化连续操作。
 
-   **画布**：实现了画布功能，支持在画布上方便的对工作流进行增加、删除、修改等操作。 

-   **提供功能**：工作流中支持了上百种功能，其中包括核密度分析、热度图等大数据分析功能，度量地理分析、模型分析等地统计分析功能，以及插值分析、表面分析等多种空间分析功能。 

-   **工作流运行**：支持运行单个节点，也支持执行整个工作流，并可对执行过程进行控制和管理。 

-   **保存**：支持将构建好的工作流保存到工作空间中，便于后续使用或修改工作流中的操作步骤及参数等。 

-   **管理**：在工作空间管理器中，新增工作流二级节点，支持对已有工作流进行重命名、删除、打开等操作。 

-   **工作流模板**：支持将已构建工作流输出为模板，可通过加载模板便捷的创建工作流。 

-   **检查**：支持检查创建的工作流程是否存在错误，如是否存在游离节点、死循环、功能无数据输入等情况。
...
-   **多版本缓存**：支持将不同的地图切成多个版本的缓存，保存到 MongoDB 中，并支持将缓存加载到地图中进行动态播放，方便用户对比、查看不同时期的数据变化。

...
public class ContentTitleBar extends HoverableShapedPanel {
	...
	public void setLeftTitleComponents(JComponent[] leftTitleComponents) {
		setLeftTitleComponents(leftTitleComponents, leftTitleComponents == null ? null : createEmptyInsets(leftTitleComponents.length));
	}
	...
	public void setRightTitleComponents(JComponent[] rightTitleComponents) {
		setRightTitleComponents(rightTitleComponents, rightTitleComponents == null ? null : createEmptyInsets(rightTitleComponents.length));
	}
	...
	private void updateLabelAlignment() {
		label.setHorizontalAlignment(labelAlignment == Alignment.LEFT ? JLabel.LEFT : labelAlignment == Alignment.RIGHT ? JLabel.RIGHT : JLabel.CENTER);
	}
		...
		if (direction.isHorizontal()) {
			...
			if (leftComponents != null) {
				for (int i = 0; i < leftComponents.length; i++) {
					...
					setConstraints(leftComponents[index], leftInsets[index], x++, 0, 1, 1, GridBagConstraints.NONE, 0, 0, GridBagConstraints.CENTER);
				}
			}
			...
			if (rightComponents != null) {
				for (int i = 0; i < rightComponents.length; i++) {
					...
					setConstraints(rightComponents[index], rightInsets[index], x++, 0, 1, 1, GridBagConstraints.NONE, 0, 0, GridBagConstraints.CENTER);
				}
			}
		} else {
			int y = 0;
			...
			if (leftComponents != null) {
				for (int i = 0; i < leftComponents.length; i++) {
					...
					setConstraints(leftComponents[index], leftInsets[index], 0, y++, 1, 1, GridBagConstraints.NONE, 0, 0, GridBagConstraints.CENTER);
				}
			}
			...
			if (rightComponents != null) {
				for (int i = 0; i < rightComponents.length; i++) {
					...
					setConstraints(rightComponents[index], rightInsets[index], 0, y++, 1, 1, GridBagConstraints.NONE, 0, 0, GridBagConstraints.CENTER);
				}
			}
		}
	...
	private void setConstraints(Component c, Insets insets, int gridx, int gridy, int gridWidth, int gridHeight, int fill, double weightx, double weighty,
			int anchor) {

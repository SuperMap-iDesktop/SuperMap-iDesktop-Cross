...
public class SimpleSplitPane extends BaseContainer {
	...
	private transient LayoutManager splitLayout = new LayoutManager() {
		@Override
		...
		public void layoutContainer(Container parent) {
			if (leftComponent == null || !leftComponent.isVisible()) {
				...
				float dividerLocationTemp = fixDividerLocation(getDividerLocation());
				int totalSize = getViewSize();
				int leftSize = (int) (totalSize * dividerLocationTemp);
		...
		public Dimension minimumLayoutSize(Container parent) {
			Dimension d = createSize(
					(leftComponent == null ? 0 : getDimensionSize(leftComponent.getMinimumSize())) + dividerSize
		...
		public Dimension preferredLayoutSize(Container parent) {
			...
			Dimension d = createSize((lv ? getDimensionSize(leftComponent.getPreferredSize()) : 0) + (lv && rv ? dividerSize : 0)
					+ (rv ? getDimensionSize(rightComponent.getPreferredSize()) : 0),
					Math.max(lv ? getOtherSize(leftComponent.getPreferredSize()) : 0, rv ? getOtherSize(rightComponent.getPreferredSize()) : 0));
			return new Dimension(d.width + getInsets().left + getInsets().right, d.height + getInsets().top + getInsets().bottom);
		}
	...
	public SimpleSplitPane(boolean horizontal, boolean heavyWeightDragIndicator) {
		...
		dividerPanel.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				if (dividerDraggable
						&& /* MouseEventCoalesceManager.getInstance().isDraggedAllowed(e) && */(e.getModifiersEx() & MouseEvent.BUTTON1_DOWN_MASK) != 0) {
					float location = (float) (getPos(dividerPanel.getLocation()) - getOffset() + getPos(e.getPoint())) / getViewSize();
	...
	private void setDragIndicator(float location) {
		...
		dragIndicator.setBounds((int) (p.getX() + getInsets().left), (int) (p.getY() + getInsets().top), (int) d.getWidth(), (int) d.getHeight());
	}
	...
	private float fixDividerLocation(float location) {
		...
		int leftSize = Math.max((int) (totalSize * location),
				leftComponent == null || !leftComponent.isVisible() ? 0 : getDimensionSize(leftComponent.getMinimumSize()));
		leftSize = Math.min(leftSize, totalSize
				- (rightComponent == null || !rightComponent.isVisible() ? 0 : getDimensionSize(rightComponent.getMinimumSize())));
	...
	private int getViewSize() {
		return getDimensionSize(getSize()) - dividerSize - (horizontal ? getInsets().left + getInsets().right : getInsets().top + getInsets().bottom);
	}
	...
	private void updateDividerCursor() {
		dividerPanel.setCursor(dividerDraggable ? new Cursor(horizontal ? Cursor.W_RESIZE_CURSOR : Cursor.N_RESIZE_CURSOR) : Cursor.getDefaultCursor());
	}

 ...
 * {@link #dispose()} method must be called when the theme is no longer needed, otherwise the native resources will not be disposed.
 * </p>
 *
 * <p>
...
public class LookAndFeelTheme extends TabbedPanelTitledTabTheme {
	...
	public static void setThemeCounter(int themeCounter) {
		LookAndFeelTheme.themeCounter = themeCounter;
	}
	...
	private void initTheme() {
		PropertyMapManager.runBatch(new Runnable() {
			...
			public void run() {
				...
				tabProps.setSizePolicy(TitledTabSizePolicy.INDIVIDUAL_SIZE).setBorderSizePolicy(TitledTabBorderSizePolicy.EQUAL_SIZE)
				...
				tabProps.getNormalProperties().getComponentProperties().setInsets(InsetsUtil.EMPTY_INSETS).setBorder(createTabInsetsBorder(false))
						.setFont(ui.getFont());
	...
	private Border createTabInsetsBorder(final boolean selected) {
		return new Border() {
			...
			public Insets getBorderInsets(Component c) {
				...
				Direction tabDirection = selected ? tab.getProperties().getHighlightedProperties().getDirection() : tab.getProperties().getNormalProperties()
						.getDirection();

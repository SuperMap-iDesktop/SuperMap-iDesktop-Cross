...
/**
 * <p>
 * A TitledTab is a tab that has support for text, icon and a custom Swing component (called title component). Titled tab supports several properties that makes
 * it possible to change the look (borders, colors, insets), layout (up, down, left, right).
 * </p>
 *
 * <p>
 * Titled tab has a line based layout, i.e. the text, icon and title component are laid out in a line. The layout of the tab can be rotated, i.e. the text and
 * the icon will be rotated 90, 180 or 270 degrees. The title component will not be rotated but moved so that the line layout will persist.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * <strong>Note:</strong> If only the normal state properties have been configured, the highlighted and disabled state will automatically use the same
 * properties as for the normal state, see {@link TitledTabProperties} and {@link TitledTabStateProperties}.
 * </p>
 *
 * <p>
 * TitledTab implements the {@link net.infonode.gui.icon.IconProvider} interface and overloads toString() so that both text and icon for the normal state is
 * shown in the tab drop down list in a tabbed panel.
 * </p>
 *
 * <p>
 * TitledTab supports mouse hovering. A {@link HoverListener} can be set in the {@link TitledTabProperties}. The hover listener receives a {@link HoverEvent}
 * when the mouse enters or exits the tab. The hover event's source will be the affected titled tab.
 * </p>
 *
...
public class TitledTab extends Tab implements IconProvider {
	...
	public static void setPanelUI(PanelUI panelUI) {
		TitledTab.panelUI = panelUI;
	}
	...
	private class StatePanel extends SimplePanel {
		...
		private void updateLayout(TitledTabStateProperties stateProperties, boolean titleComponentChanged) {
			boolean titleComponentChangedTemp = titleComponentChanged;
			if (titleComponent != null && stateProperties.getTitleComponentVisible()) {
				...
				if (titleComponentPanel.getComponentCount() == 0
						|| (titleComponentPanel.getComponentCount() > 0 && titleComponentPanel.getComponent(0) != titleComponent) || titleComponentChangedTemp
						|| gap != currentLayoutGap || alignment != currentLayoutAlignment || d != currentLayoutDirection) {
					titleComponentChangedTemp = false;
		...
		public void updateShapedPanel(TitledTabStateProperties stateProperties) {
			...
			panel.setHorizontalFlip(tabAreaOrientation == Direction.DOWN || tabAreaOrientation == Direction.LEFT ? !shapedPanelProperties.getHorizontalFlip()
		...
		public boolean updateState(Map changes, TitledTabStateProperties stateProperties) {
			...
			if (changes == null) {
				...
				label.setHorizontalAlignment(alignment == Alignment.LEFT ? JLabel.LEFT : alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.RIGHT);

				alignment = stateProperties.getVerticalAlignment();
				label.setVerticalAlignment(alignment == Alignment.TOP ? JLabel.TOP : alignment == Alignment.CENTER ? JLabel.CENTER : JLabel.BOTTOM);
				...
				if (m != null) {
					...
					if (keySet.contains(TitledTabStateProperties.ICON_TEXT_GAP)) {
						label.setIconTextGap(((Integer) ((ValueChange) m.get(TitledTabStateProperties.ICON_TEXT_GAP)).getNewValue()).intValue());
					}

					if (keySet.contains(TitledTabStateProperties.ICON_TEXT_RELATIVE_ALIGNMENT)) {
						Alignment alignment = (Alignment) ((ValueChange) m.get(TitledTabStateProperties.ICON_TEXT_RELATIVE_ALIGNMENT)).getNewValue();
						label.setHorizontalTextPosition(alignment == Alignment.LEFT ? JLabel.RIGHT : JLabel.LEFT);
					}
					...
					if (keySet.contains(TitledTabStateProperties.DIRECTION)
							|| keySet.contains(TitledTabStateProperties.TITLE_COMPONENT_TEXT_RELATIVE_ALIGNMENT)
							|| keySet.contains(TitledTabStateProperties.TITLE_COMPONENT_VISIBLE)
							|| keySet.contains(TitledTabStateProperties.TEXT_TITLE_COMPONENT_GAP) || keySet.contains(TitledTabStateProperties.ICON_VISIBLE)
							|| keySet.contains(TitledTabStateProperties.TEXT_VISIBLE)) {
						label.setDirection(stateProperties.getDirection());
	...
	private transient final TitledTabProperties properties = TitledTabProperties.getDefaultProperties();

	private transient HoverListener hoverListener = properties.getHoverListener();
	...
	private transient final StackableLayout layout;
	...
	private transient final PropertyMapTreeListener propertiesListener = new PropertyMapTreeListener() {
		@Override
	...
	private transient final PropertyChangeListener tabbedPanelPropertiesListener = new PropertyChangeListener() {
		@Override
	 ...
	 * @param titleComponent title component or null for no title component. The title component will be applied to all the states
	...
	public TitledTab(String text, Icon icon, JComponent contentComponent, JComponent titleComponent) {
		...
		eventPanel.addMouseListener(new MouseAdapter() {
			...
			private void updateFocus(TabSelectTrigger trigger) {
				if (isEnabled() && properties.getFocusable() && getTabbedPanel() != null && getTabbedPanel().getProperties().getTabSelectTrigger() == trigger) {
					Component focusedComponent = KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner();

					if (focusedComponent instanceof TitledTab && ((TitledTab) focusedComponent).getTabbedPanel() == getTabbedPanel())
		...
		addTabListener(new TabAdapter() {
			...
			public void tabAdded(TabEvent event) {
				PropertyMapWeakListenerManager.addWeakPropertyChangeListener(getTabbedPanel().getProperties().getMap(),
						TabbedPanelProperties.TAB_AREA_ORIENTATION, tabbedPanelPropertiesListener);
				updateTabAreaOrientation(getTabbedPanel().getProperties().getTabAreaOrientation());
			}

			@Override
			public void tabRemoved(TabRemovedEvent event) {
				PropertyMapWeakListenerManager.removeWeakPropertyChangeListener(event.getTabbedPanel().getProperties().getMap(),
						TabbedPanelProperties.TAB_AREA_ORIENTATION, tabbedPanelPropertiesListener);
			}
		});
	...
	private void abstractMouseMotionListener(MouseEvent e) {
		if (mouseMotionListeners != null) {
			MouseEvent event = convertMouseEvent(e);
			Object[] l = mouseMotionListeners.toArray();
			for (int i = 0; i < l.length; i++)
	 ...
	 * 
	 * <p>
	 * <strong>Note:</strong> since ITP 1.5.0 this method will change the enabled property in the {@link TitledTabProperties} for this tab. Enabled/disabled can
	 * be controlled by modifying the property or this method.
	 * </p>
	 *
	...
	private void updateBorders() {
		...
		Insets maxInsets = properties.getBorderSizePolicy() == TitledTabBorderSizePolicy.INDIVIDUAL_SIZE ? null : InsetsUtil.max(
				getBorderInsets(properties.getNormalProperties().getComponentProperties().getBorder()),
				InsetsUtil.max(getBorderInsets(properties.getHighlightedProperties().getComponentProperties().getBorder()), getBorderInsets(properties
						.getDisabledProperties().getComponentProperties().getBorder())));

		Insets normalInsets = InsetsUtil.rotate(properties.getNormalProperties().getDirection(), properties.getNormalProperties().getComponentProperties()
				.getInsets());

		Insets disabledInsets = InsetsUtil.rotate(properties.getDisabledProperties().getDirection(), properties.getDisabledProperties()
				.getComponentProperties().getInsets());

		int edgeInset = Math.min(InsetsUtil.getInset(normalInsets, tabAreaOrientation.getOpposite()),
				InsetsUtil.getInset(disabledInsets, tabAreaOrientation.getOpposite()));

		int normalLowered = Math.min(edgeInset, raised);

		Border innerNormalBorder = getInnerBorder(properties.getNormalProperties(), tabAreaOrientation, -normalLowered, maxInsets);
		Border innerHighlightBorder = getInnerBorder(properties.getHighlightedProperties(), tabAreaOrientation, raised - normalLowered, maxInsets);
		Border innerDisabledBorder = getInnerBorder(properties.getDisabledProperties(), tabAreaOrientation, -normalLowered, maxInsets);
		...
		if (changes == null) {
			...
			if (m != null) {
				...
				if (keySet.contains(TitledTabProperties.SIZE_POLICY)) {
					layout.setUseSelectedComponentSize(((TitledTabSizePolicy) ((ValueChange) m.get(TitledTabProperties.SIZE_POLICY)).getNewValue()) == TitledTabSizePolicy.INDIVIDUAL_SIZE);
				}
	...
	private Border getInnerBorder(TitledTabStateProperties properties, Direction tabOrientation, int raised, Insets maxInsets) {
		...
		Border innerBorder = new EmptyBorder(InsetsUtil.add(insets, InsetsUtil.setInset(InsetsUtil.EMPTY_INSETS, tabOrientation.getOpposite(), raised)));
		return border == null ? innerBorder : new CompoundBorder(border, innerBorder);
	}

	private Direction getTabAreaOrientation() {
		return getTabbedPanel() == null ? lastTabAreaOrientation : getTabbedPanel().getProperties().getTabAreaOrientation();
	}
	...
	private MouseEvent convertMouseEvent(MouseEvent e) {
		...
		return new MouseEvent(TitledTab.this, e.getID(), e.getWhen(), e.getModifiers(), (int) p.getX(), (int) p.getY(), e.getClickCount(), !e.isConsumed()

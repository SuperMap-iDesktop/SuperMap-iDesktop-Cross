...
/**
 * A TabbedPanelContentPanel is a component that holds a container for tab content components. It can be configured using properties that specifies the look for
 * the content panel.
 *
...
public class TabbedPanelContentPanel extends BaseContainer {
	...
	private transient final ComponentPaintChecker repaintChecker;

	private transient final PropertyMapTreeListener propertiesListener = new SetPropertyMapTreeListener();
	...
	public TabbedPanelContentPanel(final TabbedPanel tabbedPanel, JComponent component) {
		...
		shapedPanel = new HoverableShapedPanel(new BorderLayout(), tabbedPanel.getProperties().getContentPanelProperties().getHoverListener(), tabbedPanel) {
			@Override
			public boolean acceptHover(ArrayList enterableHoverables) {
				return TabbedHoverUtil.acceptTabbedPanelHover(getTabbedPanel().getProperties().getHoverPolicy(), enterableHoverables, getTabbedPanel(), this);
			}
		...
		tabbedPanel.addTabListener(new SetTabAdapter());
	}
	...
	private void update() {
		...
		InternalPropertiesUtil
				.applyTo(getProperties().getShapedPanelProperties(), shapedPanel, tabbedPanel.getProperties().getTabAreaOrientation().getNextCW());
	...
	private void repaintBorder() {
		if (repaintChecker.isPaintingOk()) {
			...
			if (d == Direction.UP)
				...
				r = new Rectangle(0, shapedPanel.getHeight() - shapedPanel.getInsets().bottom - 1, shapedPanel.getWidth(), shapedPanel.getHeight());
			else
				r = new Rectangle(shapedPanel.getWidth() - shapedPanel.getInsets().right - 1, 0, shapedPanel.getWidth(), shapedPanel.getHeight());
	...
	class SetTabAdapter extends TabAdapter {
	...
	class SetPropertyMapTreeListener implements PropertyMapTreeListener {
		...
		public void propertyValuesChanged(Map changes) {
			...
			if (m != null && m.keySet().contains(TabbedPanelContentPanelProperties.HOVER_LISTENER)) {
				shapedPanel.setHoverListener((HoverListener) ((ValueChange) m.get(TabbedPanelContentPanelProperties.HOVER_LISTENER)).getNewValue());
			}
			...
			m = (Map) changes.get(tabbedPanel.getProperties().getContentPanelProperties().getShapedPanelProperties().getMap());
			if (m != null)
				update();

			m = (Map) changes.get(tabbedPanel.getProperties().getMap());
			if (m != null && m.keySet().contains(TabbedPanelProperties.TAB_AREA_ORIENTATION)) {
				shapedPanel.setDirection(((Direction) ((ValueChange) m.get(TabbedPanelProperties.TAB_AREA_ORIENTATION)).getNewValue()).getNextCW());
			}
		}

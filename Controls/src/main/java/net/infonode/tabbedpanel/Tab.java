 ...
 *
 * <p>
 * A tab can hold a content component. The content component will then be shown in the content area of the TabbedPanel that the tab is a member of when the tab
 * is selected. If the tab doesn't have a content component, then the TabbedPanel will not show any content in the content area, i.e. it will be empty.
 * </p>
 *
 * <p>
 * The tab is basically a JPanel with a BorderLayout. The layout manager can be changed using setLayout. Components and borders can be added and removed from
 * the tab. The tab can also be subclassed to create other types of tabs, see {@link TitledTab}. <strong>In most cases {@link TitledTab} is the preferred tab
 * type to use because TitledTab adds support for a text, icon, looks etc.</strong>
 * </p>
 *
 * <p>
 * The tab component will be shown in the tab area of a TabbedPanel after the tab has become a member of that TabbedPanel by either adding or inserting it. A
 * tab can only be a member of one TabbedPanel at the same time.
 * </p>
 *
 * <p>
 * A tab can have different states when it is a member of a TabbedPanel:
 * <ul>
 * <li>Normal: This means that the tab is shown (and not selected) in the TabbedPanel. The content component is not shown until the user selects the tab.
 * <li>Highlighted: This means that for some reason the tab should be highlighted in the TabbedPanel. Highlighted could mean that the user pressed the tab with
 * the mouse and has not yet released the mouse, i.e. it has not been selected yet.
 * <li>Selected: This means that the tab is selected in the TabbedPanel. The TabbedPanel will then show the Tab's content component (if any). A selected tab
 * will also be be highlighted before it is selected.
...
public class Tab extends JPanel {
	private transient TabbedPanel tabbedPanel;
	private JComponent contentComponent;
	private JComponent focusableComponent;
	private ArrayList listeners;
	private transient DraggableComponent draggableComponent;

	private transient KeyListener focusableKeyListener = new KeyAdapter() {
		@Override
	...
	private transient TabListener tabbedPanelListener = new TabListener() {
		@Override
	 ...
	 *
	 * <p>
	 * Setting selected to true means that this tab will be the selected tab in the TabbedPanel it is a member of. If this tab is the selected tab in the
	 * TabbedPanel then setting selected to false means there will be no selected tab in the TabbedPanel until another tab is selected.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Sets a list of event components. An event component is a component in the tab that is used for internal listening to mouse events on the tab. This method
	 * makes it possible to use several components in the tab as event components.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This returns the shape of the tab. This can be be used by for example content borders in the tabbed panel so they can skip a gap where the tab intersects
	 * the tabbed panel content area.
	 * </p>
	 *
	 ...
	 *
	 * @param tabbedPanel tabbed panel that this tab is a member of or null if this tab is no longer a member o a tabbed panel
	 */

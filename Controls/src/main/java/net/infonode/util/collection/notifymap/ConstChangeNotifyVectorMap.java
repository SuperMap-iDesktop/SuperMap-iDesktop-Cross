...
public class ConstChangeNotifyVectorMap extends AbstractConstChangeNotifyMap {
		...
		if (getChangeSignalInternal().hasListeners()) {
			...
			for (ConstMapIterator iterator = map.constIterator(); iterator.atEntry(); iterator.next()) {
				...
				if (value == null) {
					...
					changes.put(iterator.getKey(), new ValueChange(getValue(iterator.getKey(), index + 1, getMapCount()), mapValue));
				}
			}
	...
	private void addMapListener(int index) {
		...
		SignalListener mapListener = new SignalListener() {
			...
			public void signalEmitted(Signal signal, Object object) {
				abstractMapLisenter(map, object);
			}
		};
	...
	private void abstractMapLisenter(final ConstChangeNotifyMap map, Object object) {
		...
		for (ConstMapIterator iterator = changes.constIterator(); iterator.atEntry(); iterator.next()) {
			...
			if (value == null) {
				...
				changes2.put(iterator.getKey(),
						vc.getOldValue() == null ? new ValueChange(getValue(iterator.getKey(), index + 1, getMapCount()), vc.getNewValue())
								: vc.getNewValue() == null ? new ValueChange(vc.getOldValue(), getValue(iterator.getKey(), index + 1, getMapCount())) : vc);
			}
		}

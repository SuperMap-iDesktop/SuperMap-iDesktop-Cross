...
import com.supermap.desktop.ui.docking.internal.HeavyWeightDragRectangle;
...
import com.supermap.desktop.ui.docking.model.*;
...
/**
 * The root window is a top level container for docking windows. Docking windows can't be dragged outside of their root window. The property values of a root
 * window is inherited to the docking windows inside it.
 *
...
public class RootWindow extends DockingWindow implements ReadWritable {
			...
			for (int i = 0; i < windowBars.length; i++) {
				...
				if (panel.isVisible()) {
					...
					if (dir == Direction.RIGHT) {
						...
						int width = Math.min(panel.getPreferredSize().width, maxWidth + windowBars[dir.getValue()].getInsets().left);
						...
						int width = Math.min(panel.getPreferredSize().width, maxWidth + windowBars[dir.getValue()].getInsets().right);
						...
						int height = Math.min(panel.getPreferredSize().height, maxHeight + windowBars[dir.getValue()].getInsets().top);
						...
						int height = Math.min(panel.getPreferredSize().height, maxHeight + windowBars[dir.getValue()].getInsets().bottom);
	...
	private transient final ViewSerializer viewSerializer;
	...
	private transient final Runnable modelCleanUpEvent = new Runnable() {
		@Override
		public void run() {
			if (cleanUpModel) {
				cleanUpModel = false;
	...
	public RootWindow(boolean heavyweightSupport, ViewSerializer viewSerializer) {
		...
		addTabMouseButtonListener(new MouseButtonListener() {
			...
			public void mouseButtonEvent(MouseEvent event) {
				abstractTabMouseButtonLisenter(event);
			}

		});
	}

	private void abstractTabMouseButtonLisenter(MouseEvent event) {
		if (event.isConsumed())
			return;

		DockingWindow windowTemp = (DockingWindow) event.getSource();

		if (event.getID() == MouseEvent.MOUSE_PRESSED && event.getButton() == MouseEvent.BUTTON1 && !event.isShiftDown() && windowTemp.isShowing()) {
			RestoreFocusWindowAction.INSTANCE.perform(windowTemp);
		} else if ((event.getID() == MouseEvent.MOUSE_CLICKED && event.getButton() == MouseEvent.BUTTON1) && (event.getClickCount() == 2)) {
			if ((windowTemp.getWindowParent() instanceof WindowBar) && getRootWindowProperties().getDoubleClickRestoresWindow())
				RestoreWithAbortWindowAction.INSTANCE.perform(windowTemp);
			else {
	...
	/**
	 * Returns the direction of the closest enabled window bar to a docking window. The distance is measured from the window edge that is furthest away from the
	 * bar.
	 *
	...
	public Direction getClosestWindowBar(DockingWindow window) {
		...
		int[] distances = new int[] { getWindowBar(Direction.UP).isEnabled() ? pos.y + window.getHeight() : Integer.MAX_VALUE,
	...
	/**
	 * <p>
	 * Creates and shows a floating window with the given window as top-level window in the floating window or without any top-level window i.e. empty floating
	 * window.
	 * </p>
	 *
	 * <p>
	 * <strong>Note 1:</strong> The created floating window is not visible per default. To make it visible, call {@link FloatingWindow}
	 * .getTopLevelAncestor().setVisible(true);
	 * </p>
	 *
	 * <p>
	 * <strong>Note 2:</strong> Floating windows are dynamically created when a window is undocked and closed/removed when all windows inside the floating
	 * window has been removed (i.e. cloased/docked/undocked to another floating window) from the floating window. The root window has a refernce to the
	 * floating window as long as the floating window has windows inside it i.e. it is not necessary to keep references to the floating window because the root
	 ...
	 * @param window the docking window that is the top level window in this floating window or null for no top-level window i.e. empty floating window
	...
	FloatingWindow createFloatingWindow(DockingWindow window, Point p) {
		...
		if (size.width <= 0 || size.height <= 0) {
			...
			size = new Dimension(Math.max(FLOATING_WINDOW_MIN_WIDTH, preferred.width), Math.max(FLOATING_WINDOW_MIN_HEIGHT, preferred.height));
		}
	...
	public DockingWindow getChildWindow(int index) {
		return index < 4 ? windowBars[index] : window != null ? (index == 4 ? window : (DockingWindow) floatingWindows.get(index - 5))
	...
	public void write(ObjectOutputStream out, boolean writeProperties) throws IOException {
		...
		ViewWriter viewWriter = new ViewWriter() {
			...
			public void writeWindowItem(WindowItem windowItem, ObjectOutputStream out, WriteContext context) throws IOException {
				abstractViewWriter(windowItem, out, context);
			}
	...
	private void abstractViewWriter(WindowItem windowItem, ObjectOutputStream out, WriteContext context) throws IOException {
	...
	private void newInternalRead(ObjectInputStream in, ReadContext context) throws IOException {
		...
		try {
			int viewCount = in.readInt();
			final View[] viewsTemp = new View[viewCount];

			for (int i = 0; i < viewCount; i++) {
				viewsTemp[i] = View.read(in, context);

				if (viewsTemp[i] != null)
					viewsTemp[i].setRootWindow(this);
			}
			...
			ViewReader viewReader = new ViewReader() {
				...
				public View readView(ObjectInputStream in, ReadContext context) throws IOException {
					int id = in.readInt();
					return id == -1 ? null : (View) viewsTemp[id];
				}
			};
	 ...
	 *
	 * @param in the stream from which to read the state
	 * @param readProperties true if the property values for all child windows should be read. This parameter can be set to true or false regardless of if the
	 *            property values was included when the state was written, though obviously no property values are read if there aren't any in the stream.
	 * @throws IOException if there is a stream error
	 */
	...
	/**
	 * Sets the maximized window in this root window. This method takes the window component and displays it at the top in the root window. It does NOT modify
	 * the window tree structure, ie the window parent remains the unchanged.
	 *
	...
	/**
	 * Removes all internal references to a view. It's not possible to restore the view to the previous location after this method has been called.
	 *
	...
	private void createWindowBars() {
		...
		for (int i = 0; i < directions.length; i++) {
			...
			mainPanel.add(windowBars[i], new Point(directions[i] == Direction.LEFT ? 0 : directions[i] == Direction.RIGHT ? 2 : 1,
					directions[i] == Direction.UP ? 0 : directions[i] == Direction.DOWN ? 2 : 1));
	...
	protected void update() {
		...
		dragTextLabel.setFont(new Font(null, dragTextLabel.getFont().getStyle(), getFont().getSize())); // 临时修改字体设置，解决 ubuntu 乱码问题。

		if (!heavyweightSupport) {
			ShapedPanel dragRectangleTemp = (ShapedPanel) this.dragRectangle;
			InternalPropertiesUtil.applyTo(properties.getDragRectangleShapedPanelProperties(), dragRectangleTemp);

			if (dragRectangleTemp.getComponentPainter() == null)
				dragRectangleTemp.setComponentPainter(new RectangleComponentPainter(Color.BLACK, properties.getDragRectangleBorderWidth()));
		} else {
	...
	void stopDrag() {
		if (dragTextContainer.getParent() != null) {
			if (!heavyweightSupport)
				dragTextContainer.getParent().repaint(dragTextContainer.getX(), dragTextContainer.getY(), dragTextContainer.getWidth(),
		...
		if (dragRectangle.getParent() != null) {
			if (!heavyweightSupport)
				dragRectangle.getParent().repaint(dragRectangle.getX(), dragRectangle.getY(), dragRectangle.getWidth(), dragRectangle.getHeight());
	...
	void setDragText(Point textPoint, String text) {
		if (textPoint != null) {
			JRootPane currentDragRootPaneTemp = getCurrentDragRootPane();

			if (reparent(currentDragRootPaneTemp.getLayeredPane(), dragTextContainer))
				currentDragRootPaneTemp.getLayeredPane().setLayer(dragTextContainer, JLayeredPane.DRAG_LAYER.intValue() + (heavyweightSupport ? -1 : 1));

			dragTextLabel.setText(text);
			dragTextContainer.setSize(dragTextContainer.getPreferredSize());
			Point p2 = SwingUtilities.convertPoint(currentDragRootPaneTemp, textPoint, dragTextContainer.getParent());
			...
			// Drag text window
			Point rp = SwingUtilities.convertPoint(currentDragRootPaneTemp, textPoint, getRootPane());
			...
			if (!getRootPane().contains(rp) && !floatingWindowsContainPoint(rp)) {
				...
				if (dragTextWindow != null) {
					...
					SwingUtilities.convertPointToScreen(textPoint, currentDragRootPaneTemp);
					dragTextWindow.setLocation(textPoint.x - dragTextContainer.getWidth() / 2, textPoint.y - dragTextContainer.getHeight() + yLocationOffs);
	...
	void setDragRectangle(Rectangle rect) {
		if (rect != null) {
			if (reparent(getCurrentDragRootPane().getLayeredPane(), dragRectangle))
				getCurrentDragRootPane().getLayeredPane().setLayer(dragRectangle, JLayeredPane.DRAG_LAYER.intValue() + (heavyweightSupport ? -1 : 0));
	...
	protected DropAction doAcceptDrop(Point p, DockingWindow window) {
		if (maximizedWindow != null) {
			...
			if (maximizedWindow.contains(p2) && getChildDropFilter().acceptDrop(new ChildDropInfo(window, this, p, maximizedWindow))) {

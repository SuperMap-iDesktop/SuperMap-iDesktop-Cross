...
import com.supermap.desktop.ui.docking.theme.internal.laftheme.TitleBarUI;
import com.supermap.desktop.ui.docking.theme.internal.laftheme.TitleBarUIListener;
...
/**
 * <p>
 * An <strong>experimental</strong> theme that tries to replicate the look of the active look and feel. This may or may not work depending on the look and feel
 * used.
 * </p>
 *
 * <p>
 * This is a theme that tries to replicate the look using the active look and feel. The tab windows will resemble the JTabbedPane look and the view title bars
 * will resemble the JInternalFrame's title bar. Note that vertical title bars might not look very nice. The theme uses heavyweight AWT components internally so
 * the {@link #dispose()} method must be called when the theme is no longer needed, otherwise the native resources will not be disposed.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * <strong>This theme is considered to be experimental and is not guaranteed to be an exact replica of the active look and feel. It is also not guaranteed to
 * work together with the active look and feel. The theme may be changed, removed etc in future versions. No support is given for the theme. This theme doesn't
 * work well with Aqua Look and Feel on Macintosh.</strong>
 * </p>
 *
 * @author johan
...
public class LookAndFeelDockingTheme extends DockingWindowsTheme {
	...
	public static int getThemeCounter() {
		return themeCounter;
	}

	public static void setThemeCounter(int themeCounter) {
	...
	public void dispose() {
		if (!disposed) {
			disposed = true;
			int count = getThemeCounter();
			count--;
			setThemeCounter(count);
			if (themeCounter == 0) {
				titleBarUI.dispose();
	...
	private void initTheme(final boolean initial) {
		PropertyMapManager.runBatch(new Runnable() {
			...
			public void run() {
				...
				ComponentPainter barContentPainter = tpTheme.getTabbedPanelProperties().getContentPanelProperties().getShapedPanelProperties()
						.getComponentPainter();
				barProps.getTabWindowProperties().getTabbedPanelProperties().getContentPanelProperties().getShapedPanelProperties().setOpaque(true)
						.setComponentPainter(new CompoundComponentPainter(new SolidColorComponentPainter(BackgroundColorProvider.INSTANCE), barContentPainter));
				...
				rootProps.getWindowAreaShapedPanelProperties().setComponentPainter(new SolidColorComponentPainter(BackgroundColorProvider.INSTANCE))
						.setOpaque(true);
				...
				// Background colors
				updateBackgroundColor(titleBarProps.getNormalProperties().getComponentProperties(), titleBarUI.getInactiveBackgroundColor());
				updateBackgroundColor(titleBarProps.getFocusedProperties().getComponentProperties(), titleBarUI.getActiveBackgroundColor());

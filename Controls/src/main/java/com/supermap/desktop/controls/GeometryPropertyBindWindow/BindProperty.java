...
public class BindProperty implements IBindProperty {
	...
	private void queryTabularTable() {
		...
		if (this.map.findSelection(true).length > 0) {
			for (int i = 0; i < this.map.findSelection(true).length; i++) {
				Recordset tempRecordset = this.map.findSelection(true)[i].toRecordset();
				int[] rows = new int[tempRecordset.getAllFeatures().size()];
				int j = 0;
				Iterator iterator = tempRecordset.getAllFeatures().keySet().iterator();
				while (iterator.hasNext()) {
					rows[j] = Integer.valueOf(iterator.next().toString());
					j++;
				}
				if (rows.length > 0) {

...
import com.supermap.desktop.ui.docking.drop.DropFilter;
...
import com.supermap.desktop.ui.docking.location.LocationDecoder;
...
import com.supermap.desktop.ui.docking.title.DockingWindowTitleProvider;
import com.supermap.desktop.ui.docking.title.SimpleDockingWindowTitleProvider;
...
/**
 * This is the base class for all types of docking windows. The windows are structured in a tree, typically with a {@link RootWindow} at the root. Each
 * DockingWindow has a window parent and a number of child windows.
 * <p>
 ...
 *
   */
	...
	private transient WindowPopupMenuFactory popupMenuFactory;
	private ArrayList mouseButtonListeners;
	private ArrayList listeners;

	private transient PropertyMapListener propertiesListener = new PropertyMapListener() {
		@Override
	...
	private transient PropertyMapTreeListener propertyObjectTreeListener = new PropertyMapTreeListener() {
		@Override
	...
	private transient WindowItem windowItem;
	private transient WeakReference lastRootWindow = new WeakReference(null);
 ...
 *
   */
	...
	/**
	 * <p>
	 * Sets the preferred minimize direction of this window. If the {@link WindowBar} in this direction is enabled this window will be placed on that bar when
	 * {@link #minimize()} is called.
	 * </p>
	 *
	 * <p>
	 * Note that a window will "remember" the last {@link WindowBar} it was added to so the preferred minimize direction is changed when the window is added to
	 * another {@link WindowBar}.
	 * </p>
	 *
	 * @param direction the preferred minimize direction of this window, null (which is default value) means use the closest, enabled {@link WindowBar}
	 * @since IDW 1.3.0
	 */
	...
	/**
	 * <p>
	 * Gets the preferred minimize direction of this window. See {@link #setPreferredMinimizeDirection(net.infonode.util.Direction)} for more information.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Adds a listener that receives mouse button events for window tabs. The listener will be called when a mouse button is pressed, clicked or released on a
	 * window tab of this window or a descendant of this window.
	 * </p>
	 *
	 * <p>
	 * The listeners are called in the reverse order they were added, so the last added listener will be called first. When all the listeners of this window has
	 * been called, the event is propagated up to the window parent of this window, if there is one.
	 * </p>
	 *
	 * <p>
	 * The {@link MouseEvent}source is the docking window connected to the tab in which the mouse event occured. The event point is the mouse coordinate where
	 * the event occured relative to the window.
	 * </p>
	 *
	...
	/**
	 * Removes a mouse button listener that has been previously added using the {@link #addTabMouseButtonListener(MouseButtonListener)}.
	 *
	 * @param listenerDocking the listener
	 * @since IDW 1.3.0
	 */
	public void removeTabMouseButtonListener(MouseButtonListener listenerDocking) {
		if ((getMouseButtonListeners() != null) && (getMouseButtonListeners().remove(listenerDocking) && getMouseButtonListeners().isEmpty())) {
	...
	void fireTabWindowMouseButtonEvent(DockingWindow window, MouseEvent event) {
		if (getMouseButtonListeners() != null) {
			MouseButtonListener[] l = (MouseButtonListener[]) getMouseButtonListeners().toArray(new MouseButtonListener[getMouseButtonListeners().size()]);
	...
	/**
	 * Splits this window in the given direction. If this window is a View which is contained in a TabWindow with a single tab, the TabWindow will splitted
	...
	public SplitWindow split(final DockingWindow splitWithWindow, final Direction direction, final float dividerLocation) {
		...
		optimizeAfter(splitWithWindow.getWindowParent(), new Runnable() {
			...
			public void run() {
				...
				w.setWindows(direction == Direction.DOWN || direction == Direction.RIGHT ? DockingWindow.this : splitWithWindow, direction == Direction.UP
						|| direction == Direction.LEFT ? DockingWindow.this : splitWithWindow);
	...
	/**
	 * Same as {@link #restore()}, but the {@link DockingWindowListener#windowRestoring(DockingWindow)} method of the window listeners will be called before
	 * restoring the window, giving them the possibility to abort the restore operation.
	 *
	...
	/**
	 * Restores this window to the location before it was minimized, maximized or closed. If the window can't be restored to the exact same location, a good
	 * approximation is performed. It's not guaranteed that the window is shown anywhere after this method has returned.
	 */
	public void restore() {
		if (isMaximized())
	 ...
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowClosed(DockingWindow)} method of all the listeners of this window and all window ancestors.
	 * The listeners of child windows will not be notified, for example closing a tab window containing views will not notify the listeners of views in that tab
	 * window.
	 * </p>
	 */
	public void close() {
	...
	/**
	 * Same as {@link #close()}, but the {@link DockingWindowListener#windowClosing(DockingWindow)} method of the window listeners will be called before closing
	 * the window, giving them the possibility to abort the close operation.
	 *
	...
	public void closeWithAbort() throws OperationAbortedException {
		WindowClosingEvent evt = new WindowClosingEvent(this);
		fireWindowClosing(evt);
		if (!evt.isCancel()) {
			close();
		}
	}
	...
	/**
	 * <p>
	 * Undocks this window from it's window parent i.e. creates a {@link FloatingWindow} containing this window.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowUndocked(DockingWindow)} method of all the listeners of this window and all window
	 * ancestors. The listeners of child windows will not be notified, for example undocking a tab window containing views will not notify the listeners of
	 * views in that tab window.
	 * </p>
	 *
	...
	/**
	 * Same as {@link #undock(Point)}, but the {@link DockingWindowListener#windowUndocking(DockingWindow)} method of the window listeners will be called before
	 * undocking the window, giving them the possibility to abort the undock operation.
	 *
	 * @param location floating window location in screen coordinates
	 * @return the floating window containing the undocked window
	 ...
	 *
	 * <p>
	 * If the window can't be docked to the exact same location, a good approximation is performed. It's not guaranteed that the window is shown anywhere after
	 * this method has returned.
	 * </p>
	 *
	 * <p>
	 * This method will call the {@link DockingWindowListener#windowDocked(DockingWindow)} method of all the listeners of this window and all window ancestors.
	 * The listeners of child windows will not be notified, for example docking a tab window containing views will not notify the listeners of views in that tab
	 * window.
	 * </p>
	 *
	 * @since IDW 1.4.0
	...
	public void dock() {
		if (isUndocked()) {
			...
			if (!dockedViews.isEmpty() && ((DockingWindow) dockedViews.get(0)).getRootWindow() != null)
				FocusManager.focusWindow((DockingWindow) dockedViews.get(0));
		}
	}

	/**
	 * Same as {@link #dock()}, but the {@link DockingWindowListener#windowDocking(DockingWindow)} method of the window listeners will be called before docking
	 * the window, giving them the possibility to abort the dock operation.
	 *
	...
	/**
	 * Returns the popup menu factory for this window. If it's null the window parent popup menu factory will be used when the mouse popup trigger is activated
	 * on this window.
	 *
	...
	/**
	 * Sets the popup menu factory for this window. If it's not null a popup menu will be created and shown when the mouse popup trigger is activated on this
	 * window.
	 *
	 ...
	 *
	 * @return the child window that last contained focus, null if no child window has contained focus or the child has been removed from this window
	 */
	...
	/**
	 * Maximizes this window in its root window or in its floating window. If this window has no root window nothing happens. This method takes the window
	 * component and displays it at the top in the root window or in the floating window. It does NOT modify the window tree structure, ie the window parent
	...
	/**
	 * Same as {@link #maximize()}, but the {@link DockingWindowListener#windowMaximized(DockingWindow)} method of the window listeners will be called before
	 * maximizing the window, giving them the possibility to abort the maximize operation.
	 *
	 ...
	 * {@link #setPreferredMinimizeDirection(net.infonode.util.Direction)} and {@link #getPreferredMinimizeDirection()}. If the {@link WindowBar} in that
	 * direction is not enabled, or the direction is null, thiw window is placed on the closest enabled {@link WindowBar}. If no suitable {@link WindowBar} was
	...
	/**
	 * Minimizes this window to a {@link WindowBar}located in <tt>direction</tt>. If no suitable {@link WindowBar}was found or this window already is minimized,
	...
	/**
	 * Same as {@link #minimize()}, but the {@link DockingWindowListener#windowMinimizing(DockingWindow)} method of the window listeners will be called before
	 * minimizing the window, giving them the possibility to abort the minimize operation.
	 *
	...
	/**
	 * Same as {@link #minimize(Direction)}, but the {@link DockingWindowListener#windowMinimizing(DockingWindow)} method of the window listeners will be called
	 * before minimizing the window, giving them the possibility to abort the minimize operation.
	 *
	 * @throws OperationAbortedException if the minimize operation was aborted by a window listener
	...
	private void doMinimize() {
		doMinimize(windowItem.getLastMinimizedDirection() != null && getRootWindow().getWindowBar(windowItem.getLastMinimizedDirection()).isEnabled() ? windowItem
				.getLastMinimizedDirection() : getRootWindow().getClosestWindowBar(this));
	}
	...
	public boolean isMinimizable() {
		return getOptimizedWindow().getWindowProperties().getMinimizeEnabled() && !isUndocked() && getRootWindow() != null
	...
	protected DockingWindow internalReplaceChildWindow(final DockingWindow oldWindow, final DockingWindow newWindow) {
		...
		optimizeAfter(newWindow, new Runnable() {
			@Override
			public void run() {
				abstractOptimizeAfterLisenter(oldWindow, newWindow, oldAncestors, nw);
			}

			private void abstractOptimizeAfterLisenter(final DockingWindow oldWindow, final DockingWindow newWindow, final WindowAncestors oldAncestors,
					final DockingWindow nw) {
				if (nw == oldWindow)
					return;
	...
	/**
	 * Makes this window visible. This causes the tabs of all {@link TabWindow} parents containing this window to be selected.
	 *
	 * @since IDW 1.1.0
	 */
	...
	/**
	 * Requests that the last focused child window becomes visible and that focus is restored to the last focused component in that window. If no child window
	 * has had focus or the child window has been removed from this window, focus is transferred to a child component of this window.
	 *
	 * @since IDW 1.1.0
	 */
	...
	private void fireWindowRemoved(DockingWindow removedFromWindow, DockingWindow removedWindow) {
		if (getListeners() != null) {
			DockingWindowListener[] l = (DockingWindowListener[]) getListeners().toArray(new DockingWindowListener[getListeners().size()]);
	...
	private void fireWindowClosing(WindowClosingEvent evt) throws OperationAbortedException {
		if (getListeners() != null) {
			DockingWindowListener[] l = (DockingWindowListener[]) getListeners().toArray(new DockingWindowListener[getListeners().size()]);

			for (int i = 0; i < l.length; i++)
				l[i].windowClosing(evt);
		}

		if (windowParent != null)
			windowParent.fireWindowClosing(evt);
	}
 ...
 *
   */
	...
	final protected DockingWindow addWindow(DockingWindow window) {
		...
		DockingWindow dockingWindow = window.getContentWindow(this);
		dockingWindow.detach();
		dockingWindow.setWindowParent(this);
		fireTitleChanged();
		dockingWindow.fireWindowAdded(this, dockingWindow);

		if (dockingWindow.isShowingInRootWindow())
			fireWindowShown(dockingWindow);
 ...
 *
   */
	...
	private Direction getSplitDirection(Point p) {
		double[] relativeDist = { p.getX() / getWidth(), (getWidth() - p.getX()) / getWidth(), p.getY() / getHeight(), (getHeight() - p.getY()) / getHeight() };
	...
	private int getEdgeDistance(Point p, Direction dir) {
		return dir == Direction.RIGHT ? getWidth() - p.x : dir == Direction.DOWN ? getHeight() - p.y : dir == Direction.LEFT ? p.x : p.y;
	}
	...
	DropAction acceptDrop(Point p, DockingWindow window) {
		...
		if (getRootWindow() != window.getRootWindow()
				|| ((window.getWindowProperties().getDockEnabled() || fw == null || fw2 != null) && (window.getWindowProperties().getUndockEnabled() || (fw == fw2)))) {
			da = !isShowing() || !contains(p) || hasParent(window) || (!getRootWindow().getRootWindowProperties().getRecursiveTabsEnabled() && insideTab()) ? null
	...
	protected DropAction split(DockingWindow window, final Direction splitDir) {
		...
		return new DropAction() {
			@Override
			public void execute(DockingWindow window, MouseEvent mouseEvent) {
				abstractDropAction(splitDir, window);
			}

		};
	}

	private void abstractDropAction(final Direction splitDir, DockingWindow window) {
		try {
 ...
 *
   */
					...
					if (parentWindow != null && parentWindow.getRootWindow() != null && !parentWindow.isMinimized() && !parentWindow.isUndocked()) {
						...
						if (w != null) {
							...
							optimizeAfter(w.getWindowParent(), new Runnable() {
								@Override
								public void run() {
									abstractOptimizeAfter(w1, fitem);
								}
							});
	...
	private void abstractOptimizeAfter(final DockingWindow w1, final WindowItem fitem) {
	...
	protected void updateWindowItem(RootWindow rootWindow) {
		windowItem.setParentDockingWindowProperties(rootWindow == null ? WindowItem.emptyProperties : rootWindow.getRootWindowProperties()

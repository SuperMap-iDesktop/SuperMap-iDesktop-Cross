...
package com.supermap.desktop.ui.docking.mouse;
...
import com.supermap.desktop.ui.docking.action.CloseWithAbortWindowAction;
import com.supermap.desktop.ui.docking.action.DockingWindowAction;

/**
 * A {@link MouseButtonListener} that performs a {@link DockingWindowAction}. The action is not performed if the mouse button event has been consumed.
 *
...
public class DockingWindowActionMouseButtonListener implements MouseButtonListener, Serializable {
	...
	private static final int MODIFIER_MASK = InputEvent.ALT_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK | InputEvent.META_DOWN_MASK;

	/**
	 * A listener that closes a window when its tab is clicked with the middle mouse button.
	 */
	public static final MouseButtonListener MIDDLE_BUTTON_CLOSE_LISTENER = new DockingWindowActionMouseButtonListener(MouseEvent.BUTTON2,
			CloseWithAbortWindowAction.INSTANCE);

	private int eventId;
	private int button;
	private int keyMask;
	private transient DockingWindowAction action;
	 ...
	 *
	 * @param button when this mouse button is clicked the action is performed , must be {@link MouseEvent#BUTTON1}, {@link MouseEvent#BUTTON2} or
	 ...
	 *
	 * @param eventId the event type for which to perform the action, must be {@link MouseEvent#MOUSE_PRESSED}, {@link MouseEvent#MOUSE_RELEASED} or
	 *            {@link MouseEvent#MOUSE_CLICKED}
	 * @param button when this mouse button for which the action is performed , must be {@link MouseEvent#BUTTON1}, {@link MouseEvent#BUTTON2} or
	 *            {@link MouseEvent#BUTTON3}
	 * @param action the action to perform
	 */
	 ...
	 * @param button when this mouse button for which the action is performed , must be {@link MouseEvent#BUTTON1}, {@link MouseEvent#BUTTON2} or
	 *            {@link MouseEvent#BUTTON3}
	 * @param keyMask the keys that must be pressed for the action to be performed, must be 0 or an or'ed combination of the key down masks found in
	 *            {@link InputEvent}.
	 * @param action the action to perform

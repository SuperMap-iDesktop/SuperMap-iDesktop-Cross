...
@Override
	...
	protected DockingWindow oldRead(ObjectInputStream in, ReadContext context) throws IOException {
		...
		DockingWindow leftWindowTemp = WindowDecoder.decodeWindow(in, context);
		DockingWindow rightWindowTemp = WindowDecoder.decodeWindow(in, context);
		super.oldRead(in, context);

		if (leftWindowTemp != null && rightWindowTemp != null) {
			setWindows(leftWindowTemp, rightWindowTemp);
			return this;
		} else
			return leftWindowTemp != null ? leftWindowTemp : rightWindowTemp != null ? rightWindowTemp : null;
	}
	@Override
	protected void updateWindowItem(RootWindow rootWindow) {
		super.updateWindowItem(rootWindow);
		((SplitWindowItem) getWindowItem()).setParentSplitWindowProperties(rootWindow == null ? SplitWindowItem.emptyPropertiesTemp : rootWindow
	...
	protected DockingWindow newRead(ObjectInputStream in, ReadContext context, ViewReader viewReader) throws IOException {
		DockingWindow leftWindowTemp = WindowDecoder.decodeWindow(in, context, viewReader);
		DockingWindow rightWindowTemp = WindowDecoder.decodeWindow(in, context, viewReader);

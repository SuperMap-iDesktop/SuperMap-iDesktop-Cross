...
import com.supermap.desktop.ui.docking.properties.ViewTitleBarStateProperties;
...
public class ViewTitleBar extends ContentTitleBar {
	private static final ButtonInfo[] buttonInfos = { new UndockButtonInfo(ViewTitleBarStateProperties.UNDOCK_BUTTON_PROPERTIES),
	...
	private transient DimensionProvider minimumSizeProvider;
	private com.supermap.desktop.ui.docking.View view;
	private AbstractButton[] buttons = new AbstractButton[buttonInfos.length];
	private transient List customBarComponents;
	...
	private transient PropertyMapListener buttonsListener = new PropertyMapListener() {
		@Override
	...
	public ViewTitleBar(View view) {
		...
		PropertyMapWeakListenerManager.addWeakTreeListener(view.getViewProperties().getViewTitleBarProperties().getMap(), propertiesListener);
		...
		if (changes == null) {
			...
			for (int i = 0; i < buttons.length; i++)
				if (buttons[i] != null) {
					...
					buttons[i].setBackground(shapedProperties.getComponentPainter() != null ? null : componentProperties.getBackgroundColor());
				}
	...
	public void updateViewButtons(Map changes) {
		InternalDockingUtil.updateButtons(buttonInfos, buttons, null, view,
				view.getViewProperties().getViewTitleBarProperties().getNormalProperties().getMap(), changes);
	...
	public void updateCustomBarComponents(List list) {
		...
		if (list != null) {
			for (int i = 0; i < list.size(); i++) {
				...
				insets[i] = i == 0 ? new Insets(0, 0, 0, 0) : new Insets(0, view.getViewProperties().getViewTitleBarProperties().getNormalProperties()
						.getButtonSpacing(), 0, 0);
			}
		}
		...
		for (int i = 0; i < buttons.length; i++) {
			...
			insets[k] = k == 0 ? new Insets(0, 0, 0, 0) : new Insets(0, view.getViewProperties().getViewTitleBarProperties().getNormalProperties()
					.getButtonSpacing(), 0, 0);
		}

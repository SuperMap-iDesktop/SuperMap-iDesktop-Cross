	...
	public SoftBlueIceDockingTheme(ColorProvider darkColor, ColorProvider lightColor, int cornerType, boolean slim) {
		...
		if (slim) {
			theme.getTabbedPanelProperties()
					...
					.setBorder(
							new FixedInsetsShapedBorder(new Insets(0, 0, 0, 0), (ShapedBorder) theme.getTabbedPanelProperties().getTabAreaProperties()
									.getComponentProperties().getBorder()));

			theme.getTabbedPanelProperties().getTabAreaProperties().getComponentProperties().setInsets(new Insets(0, 0, 0, 0));
			theme.getTabbedPanelProperties().setTabSpacing(-1);
			theme.getTabbedPanelProperties().getTabAreaComponentsProperties().getComponentProperties().setInsets(new Insets(2, 2, 2, 2));
		}
		...
		rootWindowProperties.getTabWindowProperties().getTabbedPanelProperties().addSuperObject(theme.getTabbedPanelProperties());
		rootWindowProperties.getTabWindowProperties().getTabProperties().getTitledTabProperties().addSuperObject(theme.getTitledTabProperties());

		rootWindowProperties.getShapedPanelProperties().setComponentPainter(
				theme.getTabbedPanelProperties().getTabAreaProperties().getShapedPanelProperties().getComponentPainter());

		rootWindowProperties.getTabWindowProperties().getTabbedPanelProperties().getContentPanelProperties().getShapedPanelProperties().setClipChildren(true);
		rootWindowProperties.getViewProperties().getViewTitleBarProperties().getNormalProperties().getComponentProperties().setForegroundColor(Color.BLACK)
				.setInsets(new Insets(0, 2, 0, 2));
		rootWindowProperties.getViewProperties().getViewTitleBarProperties().getNormalProperties().getShapedPanelProperties().setComponentPainter(null)
				.setOpaque(false);
		rootWindowProperties.getViewProperties().getViewTitleBarProperties().getFocusedProperties().getShapedPanelProperties().setComponentPainter(null);
		rootWindowProperties.getViewProperties().getViewTitleBarProperties().getFocusedProperties().getComponentProperties().setForegroundColor(Color.BLACK);

		TabAreaProperties p = rootWindowProperties.getWindowBarProperties().getTabWindowProperties().getTabbedPanelProperties().getTabAreaProperties();
		...
		rootWindowProperties.getWindowBarProperties().getTabWindowProperties().getTabbedPanelProperties().getContentPanelProperties()

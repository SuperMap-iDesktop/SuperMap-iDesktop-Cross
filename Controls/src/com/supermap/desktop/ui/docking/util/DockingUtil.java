...
public final class DockingUtil {
  ...
  public static FloatingWindow getFloatingWindowFor(DockingWindow window) {
	  DockingWindow windowTemp = window;
    if (windowTemp == null)
      return null;

    if (!windowTemp.isUndocked())
      return null;

    while (windowTemp != null && !(windowTemp instanceof FloatingWindow))
      windowTemp = windowTemp.getWindowParent();

		return (FloatingWindow) windowTemp;
  }
}
